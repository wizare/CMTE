...
from source.modules.decoders.h_topic_guide_rnn_decoder3 import RNNDecoder as RNNDecoder_full
from source.modules.decoders.h_topic_guide_rnn_decoder2 import RNNDecoder as RNNDecoder_noFd
...
class Seq2Seq(BaseModel):
    ...
    def __init__(self, corpus, config):
        ...
        self.t_to_feature = nn.Sequential(
            nn.Linear(self.embed_size * len(self.decoder_attention_channels) , self.hidden_size),
            nn.Tanh(),
    ...
    def encode(self, inputs, hidden=None, is_training=False):
        ...
        topic_feature = self.t_to_feature(torch.cat(topic_feature_input, dim=-1))
