...
class TextField(Field):
    ...
    def add_with_other_field(self, ofield):
        oitos = ofield.itos
        for os in oitos:
            if os not in self.itos:
                self.itos.append(os)
    ...
    def num2str(self, number):
        ...
        for w in tokens:
            if w != self.eos_token:
                ...
                break
        text = [w for w in text if w not in (self.pad_token,self.bos_token, )]
        text = " ".join(text)
        return text
...
class FixedField(TextField):
    ...
    def __init__(self,
                 fix_file='',
                 embed_file=None,):
        super(FixedField, self).__init__(embed_file=embed_file)
    ...
    def build_vocab(self):
        fr = open(self.file, 'r', encoding='utf-8')
        for line in fr:
    ...
    def str2num(self, string):
        ...
        # bow = [ ( self.stoi[w] , f) if for w, f in bow_freq.items() ]
        for w, f in bow_freq.items():
            if w in self.itos_set:
                bow.append( ( self.stoi[w] , f) )

        if len(bow) == 0:
            L= [random.randint(0, self.vocab_size-1) for _ in range(3) ]
            for i in L:
                bow.append( ( i, random.randint(1, 2))  )
