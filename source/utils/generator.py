    ...
    def decode(self, dec_state):
        ...
        for t in range(1, self.max_length+1):
            ...
            if torch.__version__ == '1.2.0':
                predecessors = ( candidates / self.V + self.pos_index.expand_as(candidates)).view(b * self.k)
            else:
                predecessors = (torch.true_divide(candidates, self.V) + self.pos_index.expand_as(candidates)).view(b * self.k).long()
