...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.src_topic_query_hidden_size = self.hidden_size * 2
        self.tgt_topic_query_hidden_size = self.hidden_size * 2
        # self.bridge_topic_query_hidden_size = self.hidden_size * 2
        ...
        self.src_attention = Attention(query_size=self.src_topic_query_hidden_size,
                                       memory_size=self.memory_size,
                                       hidden_size=self.attn_hidden_size,
                                       mode=self.attn_mode,
                                       project=False)

        self.tgt_attention = Attention(query_size=self.tgt_topic_query_hidden_size,
                                       memory_size=self.memory_size,
                                       hidden_size=self.attn_hidden_size,
                                       mode=self.attn_mode,
                                       project=False)
        ...
        self.topic_rnn_input_size = self.hidden_size + self.memory_size * 2
        self.topic_rnn = nn.GRU(input_size=self.topic_rnn_input_size,
                          hidden_size=self.hidden_size,
                          num_layers=self.num_layers,
                          dropout=self.dropout if self.num_layers > 1 else 0,
                          batch_first=True)
    ...
    def decode(self, input, state, is_training=False):
        ...
        if state_vector is None:
            ...
            state_vector = input.new_zeros(
                size=(batch_size, 1, self.hidden_size),
                dtype=torch.float)
        topic_rnn_input_list.append(state_vector)
        ...
        # guide vector 
        score = state.guide_score.unsqueeze(1)
        query = torch.cat([score, query], dim=-1)
        tgv_input_input_list = []

        #  src topic words
        if 'S' in self.attention_channels:
            src_memory = state.src_memory
            src_context, _ = self.src_attention(query=query, memory=src_memory)
            tgv_input_input_list.append(src_context)

        #  tgt topic words
        if 'T' in self.attention_channels:
            tgt_memory = state.tgt_memory
            tgt_context, _ = self.tgt_attention(query=query, memory=tgt_memory)
            tgv_input_input_list.append(tgt_context)

        # merge
        tgv_inputs = torch.cat(tgv_input_input_list, dim=-1)
        topic_guide_vector = self.tgv_layer(tgv_inputs)
        rnn_input_list.append(topic_guide_vector)
        topic_rnn_input_list.append(topic_guide_vector)
        ...
        topic_rnn_input = torch.cat(topic_rnn_input_list, dim=-1)
        topic_rnn_output, topic_rnn_hidden = self.topic_rnn(topic_rnn_input, hidden)

        h_y = self.tanh(self.fc1(rnn_hidden))
        h_topic = self.tanh(self.fc2(topic_rnn_hidden))
        k = self.sigmoid(self.fc3(torch.cat([h_y, h_topic], dim=-1)))
        new_hidden = k * h_y + (1 - k) * h_topic
        ...
        topic_input_list.append(new_hidden[-1].unsqueeze(0).transpose(0, 1))
