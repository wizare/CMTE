...
class RNNDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.topic_query_hidden_size = self.hidden_size * 2
        self.soft_prob_layer = nn.Linear(self.hidden_size * 2, 2)
    ...
    def decode(self, input, state, is_training=False):
        ...
        state.state_vector = torch.cat([new_hidden.transpose(0, 1), topic_guide_vector], dim=-1)
    ...
    def forward(self, inputs, state):
        ...
        state_vectors = inputs.new_zeros(
            size=(batch_size, max_len, self.hidden_size * 2),
            dtype=torch.float)
