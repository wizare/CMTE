...
class Topics(nn.Module):
    ...
    def init_matrix(self, matrix):
        assert len(matrix) == self.k
        assert len(matrix[0]) == self.vocab_size

        matrix = torch.tensor(matrix).transpose(0, 1)

        # num_known = 0
        # for i in range(len(matrix)):
        #     if len(matrix[i].nonzero()) == 0:
        #         nn.init.uniform_(matrix[i], -scale, scale)
        #     else:
        #         num_known += 1
        self.topic.weight.data.copy_(matrix)
